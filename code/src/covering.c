...
uint64_t *get_covering(uint64_t *mot, code64 c, int poid_min) {
	...
	mot_coset = (uint64_t*) calloc(int_par_ligne, sizeof(uint64_t));
	uint64_t limite = (1 << c.dim), cpt = 1;
	int i, j, wt;
	while (cpt < limite) {
		...
		while (j < int_par_ligne) {
			mot_coset[j] ^= c.G[i*int_par_ligne + j];
			wt += __builtin_popcountl(mot_coset[j]);
			j += 1;
		}
		j = 0;
...
int is_orphan(uint64_t *mot, code64 c, int poid_min) {
	...
	// on regarde si res à des un partout
	int j = 0;
	int int_par_ligne = (c.longueur+63) / 64;
	int wt = 0;
	while (j < int_par_ligne) {
